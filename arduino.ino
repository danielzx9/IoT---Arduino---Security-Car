#include <SPI.h>			// incluye libreria bus SPI
#include <MFRC522.h>			// incluye libreria especifica para MFRC522

#define RELAY_PIN 7
#define RST_PIN  9			// constante para referenciar pin de reset
#define SS_PIN  10			// constante para referenciar pin de slave select

MFRC522 mfrc522(SS_PIN, RST_PIN);	// crea objeto mfrc522 enviando pines de slave select y reset

byte LecturaUID[4]; 				// crea array para almacenar el UID leido
byte Usuario1[4]= {0x21, 0x08, 0xDB, 0x2B} ;    // UID de tarjeta leido en programa 1
byte Usuario2[4]= {0xFB, 0x21, 0x32, 0x09} ;    // UID de llavero leido en programa 1

void setup() {
Serial.begin(9600);			// inicializa comunicacion por monitor serie a 9600 bps
  SPI.begin();				// inicializa bus SPI
  mfrc522.PCD_Init();			// inicializa modulo lector
  pinMode(RELAY_PIN, OUTPUT); // Configurar el pin del relé como salida
}

void loop() {
    

    int notificacion = 2;
            
    char command = Serial.read(); // Leer el comando recibido

    // Cambiar el estado del relé dependiendo del comando recibido
    if (command == '0') {
      digitalWrite(RELAY_PIN, HIGH); // Encender el relé
    }  
    else if (command == '1') {
      digitalWrite(RELAY_PIN, LOW); // Apagar el relé
    }else{
      if ( ! mfrc522.PICC_IsNewCardPresent())		// si no hay una tarjeta presente
        return;						// retorna al loop esperando por una tarjeta
  
      if ( ! mfrc522.PICC_ReadCardSerial()) 		// si no puede obtener datos de la tarjeta
        return;
      for (byte i = 0; i < mfrc522.uid.size; i++) {	// bucle recorre de a un byte por vez el UID
    
      if (mfrc522.uid.uidByte[i] < 0x10){		// si el byte leido es menor a 0x10
        //Serial.print(" 0");				// imprime espacio en blanco y numero cero
        }
        else{						// sino
          //Serial.print(" ");				// imprime un espacio en blanco
          }
          //Serial.print(mfrc522.uid.uidByte[i], HEX);   	// imprime el byte del UID leido en hexadecimal
          LecturaUID[i]=mfrc522.uid.uidByte[i];   	// almacena en array el byte del UID leido      
          }
          
          //Serial.print("\t");   			// imprime un espacio de tabulacion 

          if(comparaUID(LecturaUID, Usuario1) )		// llama a funcion comparaUID con Usuario1
            notificacion = 0;
          else if(comparaUID(LecturaUID, Usuario2 ))	// llama a funcion comparaUID con Usuario2
            notificacion =1;          
          
          if(comparaUID(LecturaUID, Usuario1) )		// llama a funcion comparaUID con Usuario1
            Serial.println(notificacion);
          else if(comparaUID(LecturaUID, Usuario2 ))	// llama a funcion comparaUID con Usuario2
            Serial.println(notificacion);
           else						// si retorna falso
            //Serial.println("No te conozco"); 		// muestra texto equivalente a acceso denegado          
                  notificacion=2;
                  mfrc522.PICC_HaltA();
          notificacion=2;
    }
    
    // Esperar un corto tiempo antes de continuar (si es necesario)
    delay(10);
  
  
}

boolean comparaUID(byte lectura[],byte usuario[])	// funcion comparaUID
{
  for (byte i=0; i < mfrc522.uid.size; i++){		// bucle recorre de a un byte por vez el UID
  if(lectura[i] != usuario[i])				// si byte de UID leido es distinto a usuario
    return(false);					// retorna falso
  }
  return(true);						// si los 4 bytes coinciden retorna verdadero
}